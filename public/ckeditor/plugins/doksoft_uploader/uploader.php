...
function getUser() {
	...
	$kernel = $config['app']->make('Illuminate\Contracts\Http\Kernel');
	
	$response = $kernel->handle(
		$request = Illuminate\Http\Request::capture()
	);
	
	$id = $config['app']['encrypter']->decrypt($_COOKIE[$config['app']['config']['session.cookie']]);
	$config['app']['session']->driver()->setId($id);
	$config['app']['session']->driver()->start();
	
	if(!$config['app']['auth']->check()) return false;
	
	return $config['app']['auth']->user()->id;
}
//// END_
...
function sendResponse($uploadResult, $baseUrl) {
	...
	if ($_GET['client'] == 'plupload') {
		...
		if ($uploadResult[0] === true)
			echo "<script type=\"text/javascript\">window.parent.addFile('".$uploadResult[1][0]."','image','');window.parent.CKEDITOR.tools.callFunction(".$CKEditorFuncNum.", '".$baseUrl.$uploadResult[1][0]."', '');</script>";
		else
			echo "<script type=\"text/javascript\">window.parent.addFile('".$uploadResult[1]."','image','');window.parent.CKEDITOR.tools.callFunction(".$CKEditorFuncNum.", '', '".$uploadResult[1]. "');</script>";
...
function uploadFile(
	...
	*/
	...
	$fileExtension=explode('/',$fileMime)[1];
	
	$fileNName=rand(100,999).uniqid().'.'.$fileExtension;
	$filePath=$toDir.$fileNName;
	
    global $config;
	$db = new mysqli($config['DB_HOST'],$config['DB_USERNAME'],$config['DB_PASSWORD'],$config['DB_DATABASE']);
	if(mysqli_connect_errno()){
		echo '<script>alert("Error: DB 접속 에러!");</script>';
		exit;
	}
	$db->set_charset('utf8');
